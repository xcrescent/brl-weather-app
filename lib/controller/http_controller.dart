...
class HttpController {
  ...
  Future<Welcome> fetchWeatherForecast(double lat, double lon) async {
    // print(lat + lon);
    // final response = await http.get(Uri.parse(
    //     'https://weatherbit-v1-mashape.p.rapidapi.com/forecast/3hourly?lat=$latitude&lon=$longitude'),headers: {
    ...
    // });
    final response = await http.get(Uri.parse(
        'https://api.openweathermap.org/data/2.5/forecast?lat=$lat&lon=$lon&appid=8476478bea8d6ea40c4a303496f86447&units=metric'));
    if (response.statusCode == 200) {
      // If the server did return a 200 OK response,
      // then parse the JSON.
      // log(jsonDecode(response.body));
